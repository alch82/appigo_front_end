...
export class TCAccountService extends TCBaseService {
    ...
    getAccountInfo(): Observable<any> {
        ...
        retVal.subscribe(result => this.provideNextAccountInfo(result, true))
    ...
    updateAccount(account : TCAccount) : Observable<TCAccount> {
        ...
        const retVal = this.tcHttp
            .put(url, JSON.stringify(account), { headers : headers}).share()
            .map((response: Response) => {
                if (response.ok) {
...
// console.log(`imageBlob type: ${imageBlob.type}, size: ${imageBlob.size}`)
        let url = params.url
        let headers = new Headers({
...
// console.log(`processProfileImage(${bucket}, ${largeFileKey}, ${smallFileKey})`)
